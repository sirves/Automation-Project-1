...
describe('This is first test suite, Sirve Siniroht', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        //29.01.2024
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Roheline')
        cy.get('#lastName').type('Apelsin')
        cy.get('input[name="password"]').type('Nipitiri456')
        cy.get('[name="confirm"]').type('Nipitiri456')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        cy.get('[data-testid="phoneNumberTestId"]').type("be.empty")
        cy.get('#firstName').type('Roheline')
        cy.get('#lastName').type('Apelsin')
        cy.get('input[name="password"]').type('Nipitiri456')
        cy.get('[name="confirm"]').type('Nipitiri456')
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        cy.get('#lastName').type('Apelsin')
        cy.get('input[name="password"]').type('Nipitiri456')
        cy.get('[name="confirm"]').type('be.empty')
        cy.get('#username').type('Something')
    })
